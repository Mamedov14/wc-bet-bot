...
- Очки (по счёту основного времени): 0 — не угадал исход; 1 — угадал победителя; 2 — победитель и разница мячей;
  3 — точный счёт; ничья — 2, точный счёт ничьей — 3. Пересчёт идемпотентный: при изменении правил
  очки за сыгранные матчи пересчитываются автоматически — останется только сообщить игрокам через `/broadcast`.
...
### Админские команды (нужен `ADMIN_ID` в env)

- `/stats` — статистика игроков по завершённым ставкам: очки, среднее за ставку, % угаданных исходов,
  точные счета, доля ставок на ничью.
- `/consensus` — взвешенный прогноз толпы на ближайшие матчи: доли П1/X/П2 и топ-счета.
  Вес игрока = (очки + 1) / (ставок + 1), так что мнение стабильно угадывающих весит больше.
- `/broadcast текст` — рассылка сообщения всем активным игрокам (HTML поддерживается, можно
  несколько строк). Уходит сразу, без подтверждения.
